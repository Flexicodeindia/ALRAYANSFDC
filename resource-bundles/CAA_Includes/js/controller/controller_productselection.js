caaApp.controller('controllerProductSelection', ['$scope','$window','$filter','serviceProductSelect','serviceApplication',
		...
		serviceApplication.LoadShow('Processing...');
                ...
                serviceProductSelect.SendProductSelected($scope.Data, $scope.SessionId, $scope.HasEmail, $scope.HasMail, $scope.HasPhone, $scope.HasSMS)
                .then(
                    function(result){
                        if(result.Success)
                        {
            ...
            $scope.SuitableProducts = function(){                           
                ...
                var suitableProds = $filter('filter')($scope.products, {Category: $scope.AccountType, Access:$scope.Access,  Over16: $scope.Data.Over16 });
