caaApp.controller('controllerApplicantDetails', ['$scope','$window','serviceApplicantDetails','$interval','serviceParameters','serviceApplication',
    function ($scope,$window,serviceApplicantDetails,$interval,serviceParameters,serviceApplication) 
    {
       $scope.SaveErrors = 0;
       $scope.NewAccountData = { 'DOB':{}};
       $scope.$watch('EventLogId', function () {
		...
		{
			$scope.Data.Campaigns[i].Value = $scope.Data.Campaigns[i].Value.replace("&#39;","'");
			$scope.Data.Campaigns[i].Value = $scope.Data.Campaigns[i].Value.replace("&amp;","&");
		}
		
		//$scope.Data.Applicants[0].Marital_status_c = '';

        //$scope.Data.Applicants[0].Apply_for_Online_Banking_c  = 'No';   
...
$scope.Complete = function(){
    ...
    var data = {'Applicants' : angular.copy($scope.Data.Applicants)  , 'EventLogId' : $scope.EventLogId, 'Campaign' : $scope.Data.Campaign, 'PaperStatements' : $scope.Data.PaperStatements }
...
*/
    ...
    function(error){
     ...
     if($scope.SaveErrors == 3)
     {
		...
		//var c = new pca.Address(fields, options);
		//c.app = a;
		//c.add = i;

/*
		//c.listen("populate", function(address, variations) {
            //alert(a + i + address);
		//$scope.$digest();
		//$scope.$apply();
...
$scope.GetCampaigns = function()
{
 ...
 serviceApplicantDetails.GetCampaigns ($scope.Search)
 .then(
   function(result){
