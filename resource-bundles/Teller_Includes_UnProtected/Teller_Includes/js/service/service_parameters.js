tellerApp.service('serviceParameters', ['$q', '$rootScope', function ($q, $rootScope) {
	...
	function _customerSearchParams()
	{
		...
		{
			var rc = Enumerable.From(this.RecentCustomers);
			var existingItem = null;
			
			if(rc.Count() > 0)
			{
				var query = rc.Where('item => item.EBSId == "' + customer.EBSId + '"');
			...
			{
				var index = rc.IndexOf(existingItem);
	...
	this.AuditSearchCriteria = new _auditSearchCriteria;
