tellerApp.service('serviceCustomer', ['$q', '$rootScope', function ($q, $rootScope) {
    ...
    this.UpdateMarketingPreferences = function(accountId, hasEmail, hasMail, hasPhone, hasSMS) {
        var deferred = $q.defer();
        Teller_Core_Controller.UpdateMarketingPreferences(accountId, hasEmail, hasMail, hasPhone, hasSMS, function (result, event) {            
            if (event.status) {
                deferred.resolve(result);
            }
            else {
